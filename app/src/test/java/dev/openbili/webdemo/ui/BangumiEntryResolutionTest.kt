...
class BangumiEntryResolutionTest {
  ...
  // ── Server watch progress is authoritative ──────────────────────────

  @Test
  fun `server watch progress overrides local selection`() {
    val cardWithProgress = defaultCard.copy(
      watchProgress = BangumiWatchProgress(
        episodeId = 152L,
        episodeIndex = "152",
        positionMs = 103_000L,
        percent = 7,
    ...
    val localSelection = BangumiPlaybackStore.Selection(
      seasonId = 12345L, episodeId = 888L, bvid = "bv888"
    )

    val target = resolveBangumiEntryTarget(cardWithProgress, localSelection, allowLocalSelection = true)
  ...
  fun `server position zero is still authoritative`() {
    val cardWithZeroProgress = defaultCard.copy(
      watchProgress = BangumiWatchProgress(
        episodeId = 152L,
        episodeIndex = "152",
        positionMs = 0L,
  ...
  // ── No server record → local selection ──────────────────────────────

  @Test
  fun `falls back to local selection when no server progress`() {
    val localSelection = BangumiPlaybackStore.Selection(
      seasonId = 12345L, episodeId = 777L, bvid = "bv777"
    )
  ...
  // ── No server, no local → default card ──────────────────────────────
  ...
  // ── Invalid server episode → fall through ─────────────────────────

  @Test
  fun `server progress with episodeId zero falls through to default`() {
    val cardWithInvalidProgress = defaultCard.copy(
      watchProgress = BangumiWatchProgress(
        episodeId = 0L,
        episodeIndex = "",
        positionMs = 0L,
    ...
    val target = resolveBangumiEntryTarget(cardWithInvalidProgress, null, allowLocalSelection = false)

    // episodeId = 0L is invalid, so fall through to default
    assertEquals(999L, target.card.episodeId)
    assertEquals(0L, target.startPositionMs)
    assertFalse(target.serverResumeAuthoritative)
  }

  // ── Card stable ID does not change ─────────────────────────────────
