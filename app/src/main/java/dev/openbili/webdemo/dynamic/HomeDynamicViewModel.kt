...
class HomeDynamicViewModel : ViewModel() {
  ...
  fun ensureLoaded(loggedIn: Boolean) {
    ...
    if (!current.loading && (!current.initialized || current.error == "登录后查看关注动态"))
  ...
  fun setPageVisible(visible: Boolean) {
    ...
    val uploaderMerge = latestPortalUploaderPage?.let { page ->
  ...
  fun pollUpdates() {
    ...
    pollLoadJob = viewModelScope.launch {
      try {
        val (uploaderPage, response) =
          coroutineScope {
            val uploaderRequest =
              async(Dispatchers.IO) {
                runCatching { BiliSpaceApi.getHomeDynamicUploaders() }.getOrNull()
              }
  ...
  fun selectUploader(mid: Long?) {
    ...
    if (selectedHasUpdate && mid != null) {
      ...
      // 只有这次点击之后取回的 portal 响应才能决定下次进入的顺序。
      latestPortalUploaderPage = null
    }
  ...
  fun loadMoreUploaders() {
    val current = _state.value
    if (
      current.uploadersLoadingMore || !current.uploadersHaveMore || current.uploaderOffset.isBlank()
    )
      return
    _state.value = current.copy(uploadersLoadingMore = true)
    uploaderLoadJob?.cancel()
    uploaderLoadJob = viewModelScope.launch {
      try {
        val page =
          withContext(Dispatchers.IO) {
            BiliSpaceApi.getHomeDynamicUploaders(current.uploaderOffset)
          }
        _state.value =
          _state.value.copy(
            uploaders =
              (_state.value.uploaders + maskSessionReadUploaderUpdates(page.items)).distinctBy(
                HomeDynamicUploader::mid
              ),
  ...
  fun toggleLike(item: SpaceDynamicItem, accountMid: Long) {
    ...
    viewModelScope.launch(Dispatchers.IO) {
      val failure =
        runCatching { BiliSpaceApi.setDynamicLike(item.id, targetLiked, accountMid) }.exceptionOrNull()
      if (failure != null) {
  ...
  private fun load(reset: Boolean, refreshPortal: Boolean, silent: Boolean = false) {
    val expected = _state.value
    val expectedKey = HomeDynamicKey(expected.selectedMid, expected.videoOnly)
    val uploaderReadMid = expected.selectedMid?.takeIf { reset && it in optimisticReadUploaderMids }
    ...
    loadJob = viewModelScope.launch {
      try {
        val (baseUploaderPage, response) =
          coroutineScope {
            val uploaderRequest =
              if (refreshPortal) async(Dispatchers.IO) { BiliSpaceApi.getHomeDynamicUploaders() }
              else null
        ...
        val confirmedUploaderPage = uploaderReadMid?.let {
          withContext(Dispatchers.IO) {
            runCatching { BiliSpaceApi.getHomeDynamicUploaders() }.getOrNull()
          }
        }
        ...
        _state.value =
          current.copy(
            ...
            uploaderFirstPageIds = uploaderMerge?.firstPageIds ?: current.uploaderFirstPageIds,
  ...
  private suspend fun fetchPage(
    ...
    var response =
      if (expected.selectedMid == null) {
        BiliSpaceApi.getHomeDynamics(
          offset = if (reset) "" else expected.offset,
          videoOnly = expected.videoOnly,
        )
      } else {
        BiliSpaceApi.getHomeUploaderDynamics(
    ...
    while (filtered.size < 8 && hasMore && offset.isNotBlank() && rounds < 2) {
      val next = BiliSpaceApi.getHomeUploaderDynamics(expected.selectedMid, offset)
      filtered =
        (filtered + next.items.filter { it.video != null }).distinctBy(SpaceDynamicItem::id)
      offset = next.offset
  ...
  ): List<HomeDynamicUploader> = uploaders.map { uploader ->
  ...
  private fun mergeUploaderFirstPage(
    ...
    val refreshedItems =
      (page.items + listOfNotNull(selectedUploader) + retainedTail).distinctBy(
        HomeDynamicUploader::mid
      )
  ...
  private fun preserveSessionReadUploaderPositions(
    ...
    val merged = refreshed.filterNot { it.mid in sessionReadUploaderMids }.toMutableList()
    current
      .withIndex()
  ...
  private fun applyPendingUpdateIfAllowed() {
    if (!hostCommitAllowed || detailOverlayActive || _state.value.selectedDynamicId != null) return
    ...
    val mergedItems =
      (pending.response.items + listOfNotNull(selectedItem) + retainedTail).distinctBy(
        SpaceDynamicItem::id
      )
    val hadLoadedTail = retainedTail.isNotEmpty()
